...
test('verknüpft KEINE bestehenden lokalen Accounts (Takeover-Schutz)', () => {
  const db = buildOidcTestDb();
  // Lokaler Nutzer ohne OIDC, dessen Username zufällig der OIDC-E-Mail gleicht
  db.exec(`INSERT INTO users (username, display_name, password_hash)
           VALUES ('charlie@example.com', 'Charlie', '$2b$12$fakehash')`);
  const userinfo = { sub: 'link-sub-003', email: 'charlie@example.com', name: 'Charlie' };
  const user = findOrCreateOidcUser(db, userinfo);
  const count = db.prepare('SELECT count(*) as n FROM users').get();
  assert(count.n === 2, 'Es muss ein separater OIDC-Account angelegt werden, kein Linking');
  assert(user.oidc_sub === 'link-sub-003', `oidc_sub nicht gesetzt: ${user.oidc_sub}`);
  const local = db.prepare("SELECT * FROM users WHERE username = 'charlie@example.com'").get();
  assert(local.oidc_sub === null, 'Bestehender lokaler Account darf nicht verknüpft werden');
});
